...
class WorkspaceIndicator extends PanelMenu.Button {
    constructor(settingsManager, accentColorTracker) {
        ...
        // The extension keeps running on the lock screen (metadata
        // session-modes includes unlock-dialog) so panel state survives
        // lock/unlock without flashing -- but the squares themselves are
        // pointless there: workspaces cannot be switched from the lock
        // screen. Panel._updatePanel() only manages the shell's own
        // built-in indicators on session-mode changes (verified in the
        // extracted js/ui/panel.js: _hideIndicators iterates
        // PANEL_ITEM_IMPLEMENTATIONS), so hiding a custom status-area
        // item across modes is this actor's own job.
    ...
    _rebuild() {
        ...
        const count = workspaceManager.n_workspaces;
