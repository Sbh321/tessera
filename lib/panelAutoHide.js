...
const opacityDecl = alpha => `background-color: rgba(0, 0, 0, ${alpha});`;
const OPACITY_DECL_RE = /\s*background-color: rgba\(0, 0, 0, [0-9.]+\);/;
 ...
 * overview showing/hiding, the one trigger where poll latency shows):
 ...
 * on overview transitions (see OPACITY_DECL_RE above). At the default
 * 100 nothing is ever written and the shell fully owns its style.
 */
export class PanelAutoHideManager {
    ...
    _applyOpacity() {
        const percent = this._settingsManager.panelOpacity;
        ...
        if (percent >= 100)
            ...
            desired = base ? `${base} ${opacityDecl(percent / 100)}` : opacityDecl(percent / 100);
