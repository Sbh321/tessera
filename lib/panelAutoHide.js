...
// is active. One cheap global.get_pointer() call per tick answers the
// pointer-position input this feature needs, so a single low-frequency
// poll replaces what would otherwise be a web of barriers and hot-edge
// actors, and cannot get stuck in a stale state: whatever a tick misses,
// the next one corrects.
const POLL_INTERVAL_MS = 100;

// Keybinding (default Super+Z, customizable in Preferences) that shows/
// hides the auto-hidden panel. Grabbed only while auto-hide is active;
// toggles _keyRevealLatched, which _shouldReveal honors.
const REVEAL_KEY = 'panel-reveal-toggle';
 ...
 * when the reveal keybinding (default Super+Z) toggles it open. Off by
 * default (`panel-autohide`); with
 ...
 * - the reveal keybinding (default Super+Z) has latched it open (toggled
 *   again to release), or
...
export class PanelAutoHideManager {
    constructor(settingsManager) {
        ...
        this._keyRevealLatched = false; // reveal keybinding toggled open
        this._revealKeyBound = false;   // REVEAL_KEY currently grabbed
        this._pollId = null;
    ...
    _activate() {
        ...
        this._overviewSignalIds = [
            // Re-apply the search-entry offset on each show so it tracks
            // the current panel height, then run the reveal decision.
            Main.overview.connect('showing', () => {
                this._syncOverviewTopMargin();
                this._poll();
            }),
            Main.overview.connect('hiding', () => this._poll()),
        ];

        // The reveal keybinding is grabbed only while auto-hide is on --
        // there is nothing to reveal otherwise. Rebinding it in Preferences
        // updates the live grab automatically (Main.wm.addKeybinding watches
        // the settings key).
        this._bindRevealKey();
    ...
    _deactivate() {
        ...
        this._unbindRevealKey();
        this._keyRevealLatched = false;
        this._syncOverviewTopMargin();
    ...
    _bindRevealKey() {
        ...
        const action = Main.wm.addKeybinding(
            ...
            () => this._onRevealKey());
        if (action)
            this._revealKeyBound = true;
        else
            console.warn('tessera: failed to grab panel reveal keybinding');
    }

    _unbindRevealKey() {
        if (!this._revealKeyBound)
            return;
        Main.wm.removeKeybinding(REVEAL_KEY);
        this._revealKeyBound = false;
    }

    // Toggle show/hide and apply immediately (don't wait for the next tick).
    _onRevealKey() {
        this._keyRevealLatched = !this._keyRevealLatched;
        this._poll();
    }

    // While auto-hide is active the panelBox strut is released, so the
    // overview lays its contents out from the very top of the monitor
    // (its ControlsManagerLayout starts at getWorkAreaForMonitor().y,
    // which no longer reserves the panel) -- the "Type to search" entry
    // then sits under the revealed panel. Give the search entry a
    // top margin of ~1.9x the panel height to push it (and the stack
    // below it) clear with comfortable breathing room; the
    // bottom-anchored dash is unaffected. Cleared
    // when auto-hide turns off, since the returning strut makes the
    // overview reserve the panel on its own again. Public getter chain
    // (Main.overview.searchEntry), guarded so a future shell change only
    // ever costs this cosmetic offset, never a crash.
    _syncOverviewTopMargin() {
        try {
            ...
            entry.set_style(
                this._active
                    ? `margin-top: ${Math.round(Main.panel.height * 1.9)}px;`
                    : null);
        } catch (error) {
            // Search-entry surface changed on this shell build: skip the
            // offset (worst case the entry overlaps the panel in the
            // overview, exactly the pre-fix behavior).
        }
    }
    ...
    _shouldReveal(panelBox) {
        ...
        // Shown by the reveal keybinding (default Super+Z) until toggled off.
        if (this._keyRevealLatched)
            return true;
