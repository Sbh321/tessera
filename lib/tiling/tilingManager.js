...
export class TilingManager {
    ...
    // fullscreen, or a user-maximized member (see isExclusiveOccupant).
    // Tiling for the bucket is suspended entirely (never resize such a
    // window, never reflow beneath it -- it already covers its siblings),
    // and resumes on the notify::fullscreen / notify::maximized-* relayout
    // when the state ends.
    _bucketHasExclusiveWindow(workspace, monitorIndex) {
        ...
        return source.some(window =>
            window.get_monitor() === monitorIndex && isExclusiveOccupant(window));
    }
    ...
    _exitMaximized(workspace, exceptWindow) {
        ...
        for (const window of workspace.list_windows()) {
            if (window === exceptWindow)
                continue;
            if (isLayoutMember(window) && isMaximized(window))
                window.unmaximize(Meta.MaximizeFlags.BOTH);
        }
    }
