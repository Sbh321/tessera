...
// Helpers that open a throwaway toplevel purely to get keyboard focus
// for a moment. wl-clipboard (wl-copy / wl-paste) is the one that
// matters: Mutter offers no data-control protocol, so on GNOME it maps
// a tiny invisible xdg_toplevel, takes the selection, and destroys it
// -- all within a few milliseconds. Matched on WM class and title, both
// of which wl-clipboard sets to its own name.
const HELPER_SURFACE_NAMES = ['wl-clipboard'];

// A NORMAL toplevel this small is not an application window. The only
// real ones are the helper surfaces above (wl-clipboard maps 1x1); every
// toolkit's smallest real window is far larger. Zero is excluded on
// purpose: a Wayland window has no size at all until its first commit,
// and that must not read as "tiny".
const HELPER_SURFACE_MAX_PX = 8;

/**
 * Whether a window is a transient helper surface: something that exists
 * only to hold focus for an instant (a clipboard tool taking the
 * selection) and must not be treated as an application window by
 * anything -- not tiled, not un-maximizing its neighbours, not moved to
 * a workspace of its own, not given the focus border. Left unhandled,
 * every wl-copy split the focused tile for a frame and put it back: the
 * "the app flickers or halves when I copy" bug.
 *
 * @param {Meta.Window} window any window
 * @returns {boolean}
 */
export function isHelperSurface(window) {
    const wmClass = (window.get_wm_class() ?? '').toLowerCase();
    const title = (window.get_title() ?? '').toLowerCase();
    if (HELPER_SURFACE_NAMES.some(name => wmClass.includes(name) || title.includes(name)))
        return true;

    const frame = window.get_frame_rect();
    return frame.width > 0 && frame.width <= HELPER_SURFACE_MAX_PX &&
        frame.height > 0 && frame.height <= HELPER_SURFACE_MAX_PX;
}
...
export function isLayoutMember(window, floating = null) {
    ...
    // Clipboard helpers' momentary focus surfaces; see isHelperSurface.
    if (isHelperSurface(window))
        return false;
