 ...
 * gives it the whole monitor) or a user-maximized layout member (it
 * floats at the full work area, covering its tiled siblings). The tiler
 * treats the two identically: while a bucket has one, tiling for that
 * bucket is suspended (no sibling is reflowed beneath a window that
 * already covers it -- that reflow was visible as a brief "zoom" of the
 * other tiles, most noticeable with the top panel auto-hidden) and the
 * stacked tab bar hides (it is chrome drawn above windows and must not
 * float over a fullscreen/maximized window).
 *
 ...
 * membership so a stray maximized dialog/popup never suspends a bucket.
 *
 * @param {Meta.Window} window any window
 * @param {?Set<Meta.Window>} [floating] see isLayoutMember
 * @returns {boolean} whether it exclusively occupies its bucket
 */
export function isExclusiveOccupant(window, floating = null) {
    if (window.is_fullscreen())
        return true;
    // A user-floated maximized window is NOT exclusive: it is not a
    // member (isLayoutMember returns false for it), so it floats at its
    // maximized size over the still-tiled bucket rather than suspending
    // it. Only a *tiled* window's maximize suspends its siblings.
    return isLayoutMember(window, floating) && isMaximized(window);
}
