...
export class ActionRegistry {
    ...
    _tesseraDescriptors() {
        ...
        return [
            ...
            {
                id: 'tessera-tiled',
                title: _('Tile This Workspace'),
                subtitle: () => this._workspaceModeSubtitle('tiled'),
                iconName: 'view-grid-symbolic',
                keywords: ['tile', 'tiled', 'tiling', 'layout', 'workspace', 'dwindle'],
                run: () => tessera.tilingManager.toggleWorkspaceMode('tiled'),
            },
            {
                id: 'tessera-stacked',
                title: _('Stack This Workspace'),
                subtitle: () => this._workspaceModeSubtitle('stacked'),
                iconName: 'view-paged-symbolic',
                keywords: ['stack', 'stacked', 'tab', 'tabbed', 'layout', 'workspace'],
                run: () => tessera.tilingManager.toggleWorkspaceMode('stacked'),
            },
            {
                id: 'tessera-floating-layout',
                title: _('Float This Workspace'),
                subtitle: () => this._workspaceModeSubtitle('floating'),
                iconName: 'view-restore-symbolic',
                keywords: ['float', 'floating', 'layout', 'workspace', 'gnome', 'untile'],
                run: () => tessera.tilingManager.toggleWorkspaceMode('floating'),
            },
            {
                id: 'tessera-default-layout',
                title: _('Default Layout'),
                subtitle: () => _('Every workspace: %s · pick tiled, stacked or floating')
                    .format(this._context.settings.layoutMode),
                iconName: 'view-grid-symbolic',
                keywords: ['default', 'global', 'layout', 'mode', 'tiled', 'stacked', 'floating', 'all', 'workspaces'],
                run: () => this._cycleDefaultLayout(),
            },
            {
                id: 'tessera-floating',
    ...
    // "Currently tiled" / "Already stacked -- press to return to the
    // default" style subtitle for the three per-workspace mode actions.
    _workspaceModeSubtitle(mode) {
        const current = this._context.tessera.tilingManager.layoutModeOf();
        if (current === mode) {
            return _('Already %s — activate to return this workspace to the default (%s)')
                .format(mode, this._context.settings.layoutMode);
        }
        return _('This workspace is %s').format(current);
    }

    // The global default steps tiled -> stacked -> floating -> tiled; the
    // tiler applies it to every workspace on the setting change.
    _cycleDefaultLayout() {
        const order = ['tiled', 'stacked', 'floating'];
        const next = order[(order.indexOf(this._context.settings.layoutMode) + 1) % order.length];
        this._context.settings.gsettings.set_string('layout-mode', next);
    }
