...
// The chevron: wide enough for the glyph, no wider, so "▸ 12 tabs"
// reads as one unit at the end of the row.
const CHEVRON_WIDTH = 16;

// How far child rows step in under their parent -- enough to read as
// "belongs to" at a glance, small enough that a child's title still
// has the row's width.
...
export class LauncherTheme {
     ...
     * @param {number} [depth] 0 for a top-level row, 1 for a child row
     * @returns {string}
     */
    rowStyle(selected, depth = 0) {
        const {palette} = this;
        const {rowHeight, rowPadding, radius} = this.metrics;
        const side = rowPadding + 4;

        return buildCssDeclarations({
            'min-height': `${rowHeight}px`,
            'padding': `0 ${side}px 0 ${side + depth * CHILD_INDENT}px`,
    ...
    /**
     * The in-card toast: a dark pill with light text in both themes, so
     * it reads as a transient overlay rather than as part of the list
     * whatever the surface colour underneath it.
     */
    toastStyle() {
        return buildCssDeclarations({
            'background-color': 'rgba(40, 40, 44, 0.96)',
            'color': '#ffffff',
            'border': '1px solid rgba(255, 255, 255, 0.12)',
            'border-radius': '99px',
            'padding': '6px 14px',
            'font-size': `${Math.max(7, this.metrics.fontSize - 1)}pt`,
        });
    }

    /**
     * The quiet trailing detail on a row ("12 tabs"). Dim and small, the
     * opposite of valueStyle(): a detail about the row, not its point.
     */
    badgeStyle() {
        return buildCssDeclarations({
            'color': this.palette.dimText,
    ...
    /**
     * The small browser icon over a favicon's corner. A sliver of
     * surface-coloured backing keeps it legible over an icon of any
     * colour without reading as a second badge.
     */
    iconBadgeStyle() {
        return buildCssDeclarations({
            'background-color': this.palette.surface,
            'border-radius': '99px',
            'padding': '1px',
        });
    }
    ...
    /**
     * The disclosure chevron. Dim until hovered, like the settings gear
     * -- it is a control on the row, not part of the result -- and given
     * a fixed width so titles line up whether or not a row has one.
     *
     * @param {boolean} hover
     * @returns {string}
     */
    chevronStyle(hover) {
        ...
        return buildCssDeclarations({
            ...
            'border-radius': '4px',
            'min-width': `${CHEVRON_WIDTH}px`,
            'text-align': 'center',
            'font-size': `${this.metrics.fontSize}pt`,
            'padding': '2px 0',
        });
    }
