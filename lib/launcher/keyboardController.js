 ...
 *
 *  - Left/Right belong to the search entry, which holds key focus for
 *    the entire session, and a launcher whose text cursor cannot move
 *    is worse than one missing a shortcut. An earlier version claimed
 *    them for the filter chips whenever the cursor sat at that end of
 *    the query; even that much fought the entry in practice, so the
 *    chips moved to Ctrl+Tab -- a chord no text field wants, on the
 *    gesture people already use to change tabs in a browser or an
 *    editor. They are mapped here to EXPAND / COLLAPSE (a browser
 *    window's tabs) only because that hierarchy exists solely while the
 *    query is EMPTY -- and an empty entry has no cursor to move. The
 *    popup applies that emptiness test, exactly as for Backspace below,
 *    so with any text typed the keys still reach the entry untouched.
...
export function resolveIntent(keyval, state) {
    ...
    case Clutter.KEY_KP_Right:
        // Modified arrows are text selection / word movement in the
        // entry and must stay so; only the bare key can mean "expand".
        return control || shift || alt ? NONE : {type: IntentType.EXPAND};

    case Clutter.KEY_Left:
    case Clutter.KEY_KP_Left:
        return control || shift || alt ? NONE : {type: IntentType.COLLAPSE};
