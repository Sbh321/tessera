...
import {LauncherToast} from './toast.js';
...
class LauncherPopup extends St.Widget {
    ...
    constructor(context, controller, theme) {
        ...
        // Above the card in the tree so it draws over the list.
        this._toast = new LauncherToast(this._theme);
    ...
    open() {
        ...
        // nothing expanded. A scope left over from last time would be
        // invisible state silently hiding results.
        this._controller.setScope(null);
        this._list.resetExpansion();
    ...
    /**
     * Shows a brief confirmation inside the card ("Added to Favorites").
     * Available to every provider as `context.toast()`; a no-op while
     * the popup is closed, since there is nothing to show it over.
     *
     * @param {string} text
     * @param {{iconName?: ?string}} [options]
     */
    showToast(text, {iconName = null} = {}) {
        if (!this._isOpen)
            return;
        this._toast.show(text, {
            ...
            above: this._footer,
            iconName,
            animate: this._animationDuration > 0,
        });
    }

    /**
     * Re-runs the current query because a provider's data changed under
     * it (a browser tab opened or closed), keeping the selection where
     * it is. A no-op while closed: the next open searches afresh anyway.
     */
    refreshResults() {
        if (this._isOpen)
    ...
    _syncFooter(result) {
        ...
        const rowInfo = this._list.selectedRowInfo;
        if (rowInfo?.expandable)
            hints.push(rowInfo.expanded ? `← ${_('Collapse')}` : `→ ${_('Expand')}`);
        else if (rowInfo?.depth > 0)
            hints.push(`← ${_('Back to window')}`);
        ...
        // Named after the section it puts things in, not after the
        // mechanism: "Pin" read as a mystery until you found Favorites.
        if (result?.pinnable) {
            const pinned = this._context.favorites.isPinned(resultKey(result));
            hints.push(`Ctrl+D ${pinned ? _('Remove from Favorites') : _('Add to Favorites')}`);
        }
        if (result?.remove)
    ...
    _onKeyPress(event) {
        ...
        case IntentType.EXPAND:
        case IntentType.COLLAPSE: {
            // Only with nothing typed: with text in the entry the arrow
            // keys move the text cursor, which nothing may take away.
            // Rows that cannot expand or collapse pass the key on too,
            // so the mapping costs nothing where it does not apply.
            if (this._entry.get_text().length > 0)
                return Clutter.EVENT_PROPAGATE;
            const consumed = intent.type === IntentType.EXPAND
                ? this._list.expandSelected() : this._list.collapseSelected();
    ...
    _togglePin() {
        ...
        const pinned = this._context.favorites.toggle(resultKey(result));
        this._refreshKeepingSelection();
        this.showToast(pinned ? _('Added to Favorites') : _('Removed from Favorites'),
            {iconName: pinned ? 'starred-symbolic' : 'non-starred-symbolic'});
    }
