...
import {LauncherList} from './launcherUI.js';
...
class LauncherPopup extends St.Widget {
    ...
    applyTheme() {
        ...
        this._footer.visible = !this._context.settings.launcherCompact;
    ...
    _buildCard() {
        ...
        this._footer = new St.Label({style_class: 'tessera-launcher-footer'});
    ...
    _showSections(sections) {
        ...
        if (isEmpty) {
            // "No results" is wrong for an untouched launcher on a fresh
            // profile, where there is simply nothing to show yet.
            this._emptyLabel.text = this._entry.get_text().trim().length === 0
