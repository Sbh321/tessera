...
export class ExtensionProvider extends SearchProvider {
    ...
    /** Every installed extension, alphabetically by name. */
    browseResults() {
        return Main.extensionManager.getUuids()
            .map(uuid => Main.extensionManager.lookup(uuid))
            .filter(extension => extension?.metadata)
            .sort((a, b) => (a.metadata.name ?? a.uuid)
                .localeCompare(b.metadata.name ?? b.uuid))
            .map(extension => this._extensionResult(extension, BROWSE_SCORE, []));
    }

    browseCount() {
        return Main.extensionManager.getUuids()
            .filter(uuid => Main.extensionManager.lookup(uuid)?.metadata).length;
    }
