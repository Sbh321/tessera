...
// Width of the fade St paints over an edge that has more chips beyond
// it -- the strip's only overflow cue, in place of a scrollbar.
const FADE_WIDTH = 28;

// How far one wheel notch moves the strip, in pixels -- about a chip.
const WHEEL_STEP = 96;
...
class ScopeBar extends St.ScrollView {
    constructor(theme) {
        ...
        // scrollbar itself out of sight -- Ctrl+Tab and the wheel scroll
        // the strip, and a faded edge says there is more, so a visible
        // bar under a row of chips would be chrome nobody needs to
        // touch. Chips are never squeezed to fit: a strip of "Op…",
        // "Bro…", "App…" is not a filter bar, it is a guessing game.
        ...
        // St fades an edge only while there is content beyond it, so
        // the strip reads as "more this way" exactly when that is true.
    ...
    _chip(index) {
        ...
        if (!chip) {
            ...
            // St labels ellipsize by default, which also makes their
            // MINIMUM width a few pixels -- and a scrollable St box
            // extends only as far as its children's minimum widths, so
            // with the default the strip shrank the chips to fit the
            // card instead of scrolling. Never ellipsized, a chip's
            // minimum is its full text, and the strip grows to hold it.
            labelActor.clutter_text.ellipsize = Pango.EllipsizeMode.NONE;
            countActor.clutter_text.ellipsize = Pango.EllipsizeMode.NONE;
     ...
     * wheel is the only one most mice have, and a horizontal-only strip
