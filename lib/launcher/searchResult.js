 ...
 * @param {number[]} [fields.subtitlePositions] matched indexes in
 *   `subtitle`, for providers whose subtitle carries a searched field
 ...
 * @param {import('gi://Gio').Icon} [fields.badgeGicon] a small icon
 *   drawn over the bottom-right corner of the main icon, marking what
 *   the result belongs to (a tab's browser)
 * @param {string} [fields.badge] a short, quiet, right-aligned note
 *   drawn in small type ("12 tabs") -- the row's DETAIL rather than its
 *   point, which is what distinguishes it from `display`
 * @param {object[]} [fields.children] results nested under this one.
 *   The UI shows them, indented, only while the row is expanded; they
 *   take part in selection and activation exactly like any other row.
 *   A result with children is expandable; one without never shows a
 *   chevron. Children are never ranked or capped by the controller --
 *   they belong to their parent.
...
export function createResult(fields) {
    ...
    return {
        ...
        subtitlePositions: fields.subtitlePositions ?? [],
        keywords: fields.keywords ?? [],
        metadata: fields.metadata ?? {},
        display: fields.display ?? '',
        badge: fields.badge ?? '',
        badgeGicon: fields.badgeGicon ?? null,
        children: Array.isArray(fields.children) ? fields.children.filter(Boolean) : [],
