...
import {BrowserTabService} from './browserTabService.js';
import {BrowserTabsProvider} from './browserTabsProvider.js';
...
export class LauncherManager {
    ...
    _activate() {
        ...
        this._context = {
            ...
            // The two things a provider may do to the launcher itself,
            // rather than to the desktop: the slash palette uses them to
            // apply a filter and to complete a command into the entry.
            setScope: scope => this._popup?.setScope(scope),
            setQuery: text => this._popup?.setQuery(text),
            // A brief in-card confirmation ("Added to Favorites") for
            // anything that changes state without closing the launcher.
            toast: (text, options) => this._popup?.showToast(text, options),
        };

        this._registry = new ActionRegistry(this._context);
        this._registry.enable();

        // Browser tabs arrive from outside the shell, so their provider
        // is the one whose data changes while the popup is open; the
        // service tells the popup to re-run the current query when it
        // does. The window provider is handed the tab provider so
        // browser windows can carry their tabs as children.
        this._browserTabs = new BrowserTabService(this._settings);
        this._browserTabs.onChanged = () => this._popup?.refreshResults();
        this._context.browserTabs = this._browserTabs;
        const browserTabsProvider = new BrowserTabsProvider(this._context, this._browserTabs);

        this._controller = new SearchController(this._context);
        this._controller.setProviders([
            new AppProvider(this._context),
            new WindowProvider(this._context, browserTabsProvider),
            browserTabsProvider,
            new ActionProvider(this._context, this._registry),
            new CommandProvider(this._context),
            new PaletteProvider(this._context),
