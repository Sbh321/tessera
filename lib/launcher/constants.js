...
/**
 * Opens the slash palette, but only as the first character of the query
 * -- a "/" typed later is just a character in a search (paths, dates,
 * "and/or"). Deliberately NOT one of COMMAND_PREFIXES: those run a shell
 * command, this one opens a menu.
 */
export const PALETTE_PREFIX = '/';

/**
 * The palette's own two sections. It groups its entries by KIND rather
 * than by provider -- picking a filter and running a command are
 * different enough acts to be worth separating visually, and the split
 * is what keeps the palette legible as more commands are added.
 */
export const PALETTE_FILTERS_SECTION = 'palette-filters';
export const PALETTE_COMMANDS_SECTION = 'palette-commands';

/**
 * Sections the palette offers as filters, in the order it lists them.
 *
 * Not every section belongs here: calculator and commands only ever
 * answer an explicit prefix, and favorites only exist for an empty
 * query, so "filter to just those" would be an option that shows
 * nothing. The scope CHIPS are a different list -- they show whatever
 * the current results actually contain -- but both set the same scope
 * value.
 */
export const FILTERABLE_SECTIONS = [
    ProviderId.WINDOWS,
    ProviderId.BROWSER_TABS,
    ProviderId.APPS,
    ProviderId.ACTIONS,
    ProviderId.SETTINGS,
...
export const SECTION_ORDER = [
    PALETTE_FILTERS_SECTION,
    PALETTE_COMMANDS_SECTION,
    ProviderId.WINDOWS,
    ProviderId.BROWSER_TABS,
...
export const PROVIDER_WEIGHT = {
    ...
    // Just below windows and apps: a tab is one thing inside a window,
    // and with a few hundred of them open some tab matches almost any
    // query, so the section should not lead on a near-tie -- it leads
    // when a tab's title genuinely matches best.
    [ProviderId.BROWSER_TABS]: 0.96,
    [ProviderId.APPS]: 1.0,
