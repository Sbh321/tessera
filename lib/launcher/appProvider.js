...
// Every entry scores the same while browsing: there is no query to have
// matched better or worse, so the order is the one they are listed in,
// nudged only by the ranking boosts.
...
export class AppProvider extends SearchProvider {
    ...
    /**
     * Every installed application, alphabetically. Equal scores keep
     * that order through the controller's stable sort, so the list reads
     * as a predictable A-Z index while frecency still lifts the handful
     * you actually launch to the top of it.
     */
    browseResults() {
        return [...this._ensureEntries()]
            .sort((a, b) => a.app.get_name().localeCompare(b.app.get_name()))
            .map(entry => this._appResult(entry, {score: BROWSE_SCORE, positions: []}));
    }

    /** O(1): the cache already knows how many applications there are. */
    browseCount() {
        return this._ensureEntries().length;
    }
