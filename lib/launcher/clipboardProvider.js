...
export class ClipboardProvider extends SearchProvider {
    ...
    /**
     * The whole clipboard history, pinned entries first and the rest
     * newest-first -- the order it is stored in, which is the order it
     * is useful in.
     */
    browseResults() {
        const pinned = new Set(this._pinned);
        ...
        for (const text of [...this._pinned, ...this._history]) {
            ...
            results.push(this._entryResult(text, this._preview(text), pinned.has(text), {
                // Descending so the stored order survives the ranking
                // sort; there is no query to have matched better.
                score: 0.6 - results.length * 0.001,
                positions: [],
            }));
        }
