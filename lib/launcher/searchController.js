...
import {
    ...
    FILTERABLE_SECTIONS, MAX_RESULTS_PER_SECTION, PALETTE_PREFIX,
    PRIORITY_SECTIONS, PROVIDER_WEIGHT, ProviderId, SECTION_ORDER,
} from './constants.js';
...
export class SearchController {
    ...
    constructor(context) {
        ...
        this._scope = null;
        // How many results the unfiltered resting view holds -- the
        // "All" chip's count, remembered while gathering it.
        this._restingCount = 0;
     ...
     *   counts: Map<string, number>, total: number}} the sections to draw
     *   (filtered by the current scope), the per-section totals the scope
     *   bar needs, and what "All" itself would show. Every number
     *   describes the UNFILTERED world, so moving into one section never
     *   erases the evidence that the others have something in them.
     */
    search(raw) {
     ...
     */
    _restingResults() {
        const resting = [...this._favoriteResults()];
        for (const provider of this._activeProviders())
            resting.push(...provider.defaultResults());

        // Remembered for the scope bar's "All" count. It is NOT the sum
        // of the chips: at rest, "All" shows this curated view while
        // each section chip shows everything in that section, so the two
        // numbers count different things and both are honest.
        this._restingCount = resting.length;

        if (!this._scope)
            return resting;

        // Filterable sections are named after their provider, so the
        // scope resolves straight to the one being browsed. The resting
        // view is still gathered above -- dropping it would leave the
        // scope bar counting only the filtered section, which is what
        // used to make the other chips vanish the moment you moved to
        // one of them.
        const provider = this._providersById.get(this._scope);
        return provider?.enabled ? [...resting, ...provider.browseResults()] : resting;
    }

    /**
     * How many results each section would show if you filtered to it
     * with nothing typed -- its whole contents, not its slice of the
     * resting view.
     *
     * Counted rather than built (`browseCount()`): only one section is
     * ever drawn, so materialising three hundred application records to
     * print "312" would be pure waste.
     */
    _browseCounts() {
        const counts = new Map();

        for (const sectionId of FILTERABLE_SECTIONS) {
            const provider = this._providersById.get(sectionId);
            if (!provider?.enabled)
                continue;

            const count = provider.browseCount();
            // An empty section gets no chip -- except the one being
            // filtered to, which must stay selectable to be escapable.
            if (count > 0 || sectionId === this._scope)
    ...
    _buildSections(results, parsed) {
        ...
        //
        // With nothing typed there is no match count to report, so the
        // chips carry each section's total size instead -- what you would
        // get by filtering to it. Either way the numbers describe the
        // UNFILTERED world, which is what lets you see from inside one
        // section that another has something in it.
        const counts = parsed.isEmpty ? this._browseCounts() : new Map();
        if (!parsed.isEmpty) {
            for (const {result} of ranked)
                counts.set(result.section, (counts.get(result.section) ?? 0) + 1);
        }

        // What the "All" chip itself shows: the resting view at rest, and
        // every match while searching.
        const allCount = parsed.isEmpty ? this._restingCount : ranked.length;
        ...
        return {sections: ordered, counts, total: allCount};
    }
