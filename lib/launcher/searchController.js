...
import {
    ...
    MAX_RESULTS_PER_SECTION, PRIORITY_SECTIONS, PROVIDER_WEIGHT, ProviderId,
    SECTION_ORDER,
} from './constants.js';
...
export function parseQuery(raw, options = {}) {
    ...
    const searchable = prefix ? commandBody : trimmed;
...
export class SearchController {
    ...
    constructor(context) {
        ...
        /** @type {?function(Array<object>): void} set by the popup. */
        this.onResults = null;
    }
     ...
     * @returns {Array<{id: string, results: Array<object>}>} sections
     */
    search(raw) {
        ...
        if (parsed.isEmpty) {
            collected.push(...this._favoriteResults());
            for (const provider of this._activeProviders())
                collected.push(...provider.defaultResults());
        } else {
            for (const provider of this._activeProviders()) {
        ...
        for (const promise of pending) {
            promise.then(results => {
                ...
                this.onResults(this._buildSections(this._currentResults));
            }).catch(error => {
                console.warn(`tessera: launcher provider failed: ${error}`);
            });
        }

        return this._buildSections(this._currentResults);
    }
    ...
    _buildSections(results) {
        ...
        for (const {result, rank} of ranked) {
            ...
            if (used >= MAX_RESULTS_PER_SECTION)
                continue;
