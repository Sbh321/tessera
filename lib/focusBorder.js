...
// The corner radius the border rounds to for ordinary (non-maximized)
// windows. There is no API to read an arbitrary window's own corner
// radius, so this matches GNOME's standard client-side window rounding
// (~12px). Maximized windows square off, and so does the border.
...
class FocusBorderManager extends St.Widget {
    ...
    _sync() {
        ...
        // located_on_workspace() also guards the empty-workspace case: on
        // switching to a workspace with no windows, mutter often keeps
        // focus_window pointing at the previously focused (frequently
        // maximized) window on another workspace. Without this check its
        // full-size frame would be outlined on the now-empty workspace,
        // making the border look like it wraps the whole workspace.
        // Sticky (on-all-workspaces) windows still pass, as they should.
        ...
        // Follow the window's own corner rounding rather than a fixed
        // setting. A fully maximized window has square corners, so the
        // border squares off too; everything else rounds to GNOME's
        // standard window radius. The border sits `width` px outside the
        // frame, so its outer radius is the window radius plus the border
        // width, which makes the inner edge hug the window's corner.
        const flags = window.get_maximized();
        const fullyMaximized =
            (flags & Meta.MaximizeFlags.HORIZONTAL) &&
            (flags & Meta.MaximizeFlags.VERTICAL);
        const radius = fullyMaximized ? 0 : WINDOW_CORNER_RADIUS + width;
