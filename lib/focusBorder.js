...
import {isHelperSurface} from './tiling/windowFilter.js';
 ...
 * disabled. The corner radius is a single configurable value applied
 * uniformly to every window (there is no API to read an app's own radius).
 *
...
class FocusBorderManager extends St.Widget {
    ...
    _onFocusChanged() {
        ...
        // A clipboard helper's invisible focus surface (see
        // windowFilter.isHelperSurface) holds focus for a few
        // milliseconds. Returning BEFORE touching the tracked window is
        // what keeps the border still: an earlier version untracked
        // first and only skipped the *tracking* of the helper, which
        // left nothing tracked, hid the border, and re-showed it when
        // focus came back -- a few-millisecond blink of the border on
        // every wl-copy. The real window is still tracked and its
        // border still drawn; focus returning to it is a plain
        // re-track of the same window.
        if (window && isHelperSurface(window))
            return;
    ...
    _sync() {
        ...
        // Only border a real occupant of the CURRENT workspace. Excluding
        // on-all-workspaces (sticky) windows is what fixes the empty-
        // workspace border: a sticky window (e.g. secondary-monitor windows
        // under "workspaces only on primary", or any pinned window) counts
        // as present on every workspace -- including empty ones -- so it
        // would otherwise get a border on a workspace that looks empty. A
        // non-sticky window must literally live on the active workspace;
        // otherwise (off-workspace focus lingering after a switch, or a
        // sticky window) there is nothing here to highlight.
