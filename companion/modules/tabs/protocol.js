...
/**
 * The wire protocol between the browser companion and Tessera, plus the
 * record normalisers both ends share.
 *
 * Imported by the companion (inside the browser) AND by the shell's tab
 * store (inside gnome-shell), so it must stay free of browser globals,
 * GNOME imports and DOM APIs -- which is also what lets the protocol be
 * unit-tested under node and gjs alike.
 *
